  ...
  userRegister: async (req, res, next) => {
    ...
    const hashedPassword = await hashPassword(password);
     ...
     const user = new User({
      ...
      return res
        .status(400)
        .json({ message: `Error in Register ${err.message}` });
    }
  },
...
loginWithGoogle : async (req, res) => {
  try {
    ...
    oAuth2Client.verifyIdToken({idToken : tokenId, audience :"clientid"})
    res.json({ ok: true, message: ` A verification Code  has been sent to ${email}.Please check your Email`, })
    .then(response => {
        ...
        User.findOne({email}).exec((err,user) => {
          if(err){
            ...
            if(user){
              ...
              //if user not exit in db then create new user with that email
            }
          }
        })
      }
